...
clip_list = []
with torch.no_grad(): 
	# for name in ['-5','-3','-1','1','3','5','7','9','11','13']:  #'0','3','6','9','12' #'-5','-3','-1','1','3',  '-5','-3','-1','1','3','5','7','9','11'
	# for channel_type in ['fading_no_normalize']: ## 'awgn', 'fading', 'mimo' #
		# print(channel_type)
	with open('respond/results.txt', 'a+') as f:
		for name in ['1','3','5','7','9','11']:  #'0','3','6','9','12' #'-5','-3','-1','1','3',  '-5','-3','-1','1','3','5','7','9','11' 
			# target_dir = generated_dir+channel_type+'/'+name+'/'
...
# # 'LSUN_bedroom_512-ema_-v_scale-0_n_samples-1'     超参数的影响
# # 'bedroom_512-ema_-v_scale-0_n_samples-16'
# with torch.no_grad():
# 	for name in ['ema_no-v','nonema_no-v','ema_-v','nonema_-v']: 
# 		IS = is_value('bedroom_512-'+name+'_scale-0_n_samples-16/samples/',batch_size, device)
# 		FID = fid_value('LSUN/data/bedroom/bedroom_val_lmdb/','bedroom_512-'+name+'_scale-0_n_samples-16/samples/',batch_size, device, dims)
# 		# print('LSUN_church_512-ema_'+name)
# 		# print(IS, FID


# # 'LSUN_bedroom_512-ema_no-v_scale-0_n_samples-1'     n_samples的影响
# with torch.no_grad():
# 	for name in ['2','4','8']: 
# 		IS = is_value('LSUN_bedroom_512-ema_no-v_scale-0_n_samples-'+name+'/samples/',batch_size, device)
# 		FID = fid_value('LSUN/data/bedroom/bedroom_val_lmdb/','LSUN_bedroom_512-ema_no-v_scale-0_n_samples-'+name+'/samples/',batch_size, device, dims)
# 		# print('LSUN_church_512-ema_'+name)
# 		# print(IS, FID)

# # 'LSUN_bedroom_768-ema-model_no-v_scale-0_n_samples-1'    model的影响
# with torch.no_grad():
# 	for name in ['768-ema','768-nonema','512-nonema']: 
# 		IS = is_value('LSUN_bedroom_'+name+'-model_no-v_scale-0_n_samples-1/samples/',batch_size, device)
# 		FID = fid_value('LSUN/data/bedroom/bedroom_val_lmdb/','LSUN_bedroom_'+name+'-model_no-v_scale-0_n_samples-1/samples/',batch_size, device, dims)
# 		# print('LSUN_church_512-ema_'+name)
# 		# print(IS, FID)